    ...
    <div className="hidden lg:flex col-span-2  rounded-xl bg-[#FFFFFF]">
      <ul className="flex flex-col  gap-x-4 w-full">
        {linksDashboad.map((item: LinkItems) => (
          <li key={item.id} className=" cursor-pointer p-5 border-b  relative border-solid border-b-gray-100 text-sm hover:bg-green-50 hover:text-green-900 hover:before:contents{` `} hover:before:bg-green-950 hover:before:h-full hover:before:w-1 hover:before:absolute hover:before:right-0 hover:before:inline-block hover:before:top-0">
            <Link
              href={item.link}
